...
describe('Nitro API Dynamic Proxy Helper (TDD)', () => {
  it('도메인 전용 URL과 API 경로가 정상적으로 포매팅되어야 함', () => {
    const result = resolveProxyTarget('https://api.example.com', '/api/roles')
    expect(result).toBe('https://api.example.com/api/roles')
  })

  it('프로토콜 스키마가 없는 경우 https:// 자동 보정되어야 함', () => {
    const result = resolveProxyTarget('api.example.com', '/api/system/install-status')
    expect(result).toBe('https://api.example.com/api/system/install-status')
  })
