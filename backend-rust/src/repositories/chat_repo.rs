...
impl ChatRepository {
    pub async fn get_user_rooms(
        pool: &PgPool,
        user_id: &str,
    ) -> Result<Vec<ChatMessageRoom>, sqlx::Error> {
        let rows = sqlx::query(
            r#"
    ...
    pub async fn delete_room(pool: &PgPool, room_id: Uuid) -> Result<(), sqlx::Error> {
        sqlx::query("DELETE FROM chat_message WHERE room_id = $1")
            .bind(room_id)
            .execute(pool)
            .await?;
        sqlx::query("DELETE FROM chat_message_room_member WHERE room_id = $1")
            .bind(room_id)
            .execute(pool)
            .await?;
        sqlx::query("DELETE FROM chat_message_room WHERE id = $1")
            .bind(room_id)
            .execute(pool)
            .await?;
        Ok(())
    ...
    pub async fn delete_message(
        ...
        let room_id: Option<Uuid> =
            sqlx::query_scalar("DELETE FROM chat_message WHERE id = $1 RETURNING room_id")
                .bind(message_id)
                .fetch_optional(pool)
                .await?;
