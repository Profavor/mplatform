...
public interface RecordRepository extends JpaRepository<Record, UUID>, CustomRecordRepository {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM Record r WHERE r.node.domain.id = :domainId AND r.data LIKE %:searchText%")
