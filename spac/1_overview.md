...
> **플랫폼 기준 버전**: Backend `2.0.3` (Rust Axum 0.8 / Tokio / SQLx) / Frontend `2.0.3` (Nuxt 4 / Vue 3)
...
| **유효 필드 2중 캐싱** | EffectiveFields Hybrid Caching | 노드 기준 상속된 최종 필드 수집 결과(`getEffectiveFields`)를 **Redis(분산 캐시)**에 적재하며, Redis 장애/미연결 시 **Local In-Memory 캐시**로 자동 폴백(Fallback)하는 고성능 캐싱 체계. |
...
| **세분화 PBAC & 컬럼 마스킹** | Granular PBAC & Column Dynamic Masking | 리소스/액션 기반 세분화 권한(`domain:read`, `system:user:write`) 및 와일드카드(`*`, `domain:*`) 평가 체계, 사용자 스코프 제한 및 미인가 사용자에 대한 민감 컬럼 런타임 동적 마스킹(`***`) 체계. |
...
| 구분 | 항목 | 규모 / 스펙 | 세부 설명 |
|---|---|---|---|
| **버전 정보** | 플랫폼 릴리즈 버전 | **Backend `2.0.3` / Frontend `2.0.3`** | Rust Axum 0.8 & Nuxt 4 프로덕션 릴리즈 |
| **백엔드 도메인 모델** | Rust Struct & SQLx 모델 | **77개 (+ 9 enums)** | PostgreSQL 15 기반 마스터 데이터, 동적 스키마, 2FA, 온보딩, 거버넌스 모델 |
| **백엔드 라우터/컨트롤러**| Axum Router & Handlers | **97개+** | REST API & WebSocket 핸들러 |
| **백엔드 서비스** | 비동기 서비스 (Services) | **136개+** | 동적 상속, DQ 자율 치유, 매칭, 결재, 해시체인, 2FA, PBAC 권한, 계보 및 연계 서비스 |
| **백엔드 쿼리 레이어** | SQLx Repositories & Queries | **69개+** | 컴파일 타임 쿼리 검증, Blind Index 검색, 시계열 통계 쿼리 |
| **백엔드 테스트** | Cargo Unit/Integration Tests | **단위/통합 테스트 스위트** | PBAC 권한 매칭, 암호문 검증 및 단위/통합 테스트 통과 |
| **백엔드 메모리/성능** | Rust 런타임 효율성 | **~13.3MB RSS** | 기존 JVM 대비 173배 메모리 절감, 마이크로초 단위 응답 지연 |
| **프론트엔드 화면** | Nuxt 4 라우트 페이지 (Pages) | **28개** | 소개 랜딩, B2B 가입, ROI 계산기, 마스터 데이터, 결재, 거버넌스 전용 페이지 |
| **프론트엔드 컴포넌트** | Vue 3 UI 컴포넌트 (Components) | **170개** | AG-Grid Community (Infinite Model), Drawer, 계보 시각화 모달, 차트 등 고성능 재사용 컴포넌트 |
| **프론트엔드 비즈니스 훅**| Composables & Stores | **28개 Composables + 6개 Stores**| PBAC 권한 훅(`usePermission`), 인증 갱신, 다국어/타임존, 웹소켓 |
